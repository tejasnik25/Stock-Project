  ...
  const run = async (save: boolean) => {
    ...
    try {
      const res = await fetch('/api/analysis/ai', {
        ...
        body: JSON.stringify({ strategy, inputs: { fifteenMin, oneHour, fiveMin }, save, userId: save ? user?.id : undefined }),
      });
      const data: AnalysisResult | { error?: string } = await res.json();
      if (!res.ok) throw new Error((data as { error?: string }).error || 'Failed');
      setResult(data as AnalysisResult);
      const savedId = (data as AnalysisResult)?.id;
      if (save && savedId) {
        // redirect to analysis detail if saved
        router.push(`/analysis/${savedId}`);
      }
    } catch (e: unknown) {
